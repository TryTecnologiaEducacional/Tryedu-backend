  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST' && !is_null($rs->Token) && $rs->Token == $Token || ($tabela == 'AccessLevel' && $acao == 'consulta')){
      ...
      switch ($acao) {
        ...
        case 'consultaComResposta':
          ...
          while($rs = $reg->fetchObject()){
            ...
            if($tabela == 'PlanosDeAula'){
              ...
              if ($ra) {
                $regA = $ObjAnswers->listar($f);
                $rsA = $regA->fetchObject();
                $tmp['Respondida'] = (is_null($rsA->Answer))? false : true;
                $tmp['VideoViewed'] = $rsA->VideoViewed;
              } else {
                $tmp['Respondida'] = false;
                $tmp['VideoViewed'] = false;
              }
            }
            array_push($resp,$tmp);
          }
        ...
        case 'create':
          ...
          if($qtd == 0){
          ...
          } elseif (stristr($tabela, 'Answers')) {
            $regA = $ObjBd->listar("idUser = $idUser AND idQuestions = ".$_POST['idQuestions']);
            $chave = $regA->fetchObject()->idQuestions;
            if ($ObjBd->atualizar($chave,$_POST) > 0){
              ...
              $resp['mensage'] = 'erro ao atualizar dados';
            }
          }else{
